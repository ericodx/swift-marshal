...
struct InitCommandTests {
    ...
    func createsConfigFileWhenNotExists() throws {
        ...
        let configPath = tempDir + "/.swift-marshal.yaml"
    ...
    @Test(
        "Given an SPM project directory, when executing init command, then config includes Sources path"
    )
    func createsSpmConfigWithSourcesPath() throws {
        let tempDir = createTempDirectory()
        defer { removeTempDirectory(tempDir) }
        ...
        let content = try String(contentsOfFile: tempDir + "/.swift-marshal.yaml", encoding: .utf8)
        #expect(content.contains("paths:"))
        #expect(content.contains("- Sources"))
    }

    @Test(
        "Given an Xcode project directory, when executing init command, then config includes project name as path"
    )
    func createsXcodeConfigWithProjectPath() throws {
        let tempDir = createTempDirectory()
        defer { removeTempDirectory(tempDir) }
    ...
    @Test(
        "Given an unknown project directory, when executing init command, then config has no paths section"
    )
    func createsConfigWithoutPathsForUnknownProject() throws {
        let tempDir = createTempDirectory()
        defer { removeTempDirectory(tempDir) }
    ...
    @Test(
        "Given an SPM project, when parsing the generated config, then paths contains Sources"
    )
    func spmGeneratedConfigIsValidAndContainsPaths() throws {
        let command = InitCommand()
        let content = command.configContent(for: .spm(sourcesPath: "Sources"))

        let loader = ConfigurationLoader()
        let mapper = ConfigurationMapper()
        let config = mapper.map(try loader.parse(content))
    ...
    @Test(
        "Given an Xcode project, when parsing the generated config, then paths contains the project name"
    )
    func xcodeGeneratedConfigIsValidAndContainsPaths() throws {
        let command = InitCommand()
        let content = command.configContent(for: .xcode(sourcesPath: "MyApp"))

        let loader = ConfigurationLoader()
        let mapper = ConfigurationMapper()
        let config = mapper.map(try loader.parse(content))

        #expect(config.paths == ["MyApp"])
        #expect(config.memberOrderingRules.count == 10)
    }
    ...
    func unknownFlagThrowsError() {
        #expect(throws: ArgumentParsingError.self) {
            try InitCommand.parse(["--unknown-flag"])
        }
    }
